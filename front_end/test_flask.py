...
CONFIG_FILE="run_conf.properties" #run from script so basedir is there
CORE = read_conf(CONFIG_FILE)
SEARCH="search"
SEARCH_TITLE="normal"
SEARCH_PR="search_pr"
SEARCH_PR_TITLE="pagerank"
...
def search():
    return search_frame( SEARCH_TITLE, SEARCH, request, CORE )

@app.route("/" + SEARCH_PR, methods=['POST', 'GET'])
def search_pr():
    return search_frame( SEARCH_PR_TITLE, SEARCH_PR, request, CORE, [("sort", "pageRankFile desc")])
